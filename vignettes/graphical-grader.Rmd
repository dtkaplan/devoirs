---
title: "Using the `{devoirs}` Essay Grader"
...
There is currently only one way to score essays submitted by students: the Essay Grader shiny app. To use this you must:

1. One time for each course ... Create a *grading directory* as described in @sec-grading-directory.

2. Make sure there are some submissions on the *collection site* where students upload their work.

3. In the console, start the Essay Grader app with `devoirs::ViewEssays()`.

You will likely return to the grader app again and again as new submissions come in over the days of your course. 

4. You can (optionally) assign *scores* to any essay. These scores are saved in the Scores.RDS file in the grading directory. When you return to an essay that was previously scored, that score will appear alongside the essay. You can, of course, change it if you have reconsidered the original score, but you do not have to.

5. To assemble your gradebook, use data wrangling on Scores.RDS to add up student scores. I NEED to provide a function for this.
...
collection site
: A facility, set up by the course instructor, where students can upload (or simply "paste") the JSON string from their work with course HTML files. This will typically be a Google spreadsheet that students can write to (via a form) and only instructors can read from or edit. 
...
The new submissions need to be made available as a CSV file. For example, with Google Forms, you can collect the submissions into a Google Sheet, which you can then publish to the web and access via a URL generated by Google. (Security comes from not giving the URL to anyone who shouldn't have it, like students!) Other collection systems have their own ways of providing such functionality, but even something as primitive as downloading a CSV to your laptop will work. 
...
- `JSON_STORE.RDS` a file established and maintained by `{devoirs}` to hold all of the student submissions that have been downloaded from the collection site. You do not need to work with this file directly. DO NOT DELETE it unless you are starting over with a new course, new semester, and so on.
...
```r
devoirs::grade_essays()
```

This will display a dialog box to allow you to select a grading directory. (If you haven't yet made the grading directory for your course, see @sec-grading-directory.) Then a browser window will be opened containing the graphical grader. There is a menu at the top of the window with tabs for [Essays], and [Close & Update]. The [Essays] tab will be open at first. If 
...
THIS IS NOT YET IMPLEMENTED. Use the Score.RDS file to access the MC scores, which are generated automatically.
...
Another app will be made available to read the scores stored in the `Scores.RDS` file, weight them, and construct a gradebook file. 
