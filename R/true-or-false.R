...
#' not recommended.
#' @param leed String like "yes or no?" Default: "True or False"
...
devoirsTF <- function(correct = FALSE, id = NULL,
  ...
  yaml.code <- c(glue::glue("#| label: {id}"),
    "#| inline: true",
    glue::glue("#| show_hints: {ifelse(grepl('none', answer_style()), 'false', 'true')}"),
    glue::glue("#| leed: {leed}"))
  ...
  Res <- list(leed = leed,
              ...
              code = c(
                glue::glue(
                  "1. {left_label} {ifelse( correct, right.hint, wrong.hint)}"),
                glue::glue(
